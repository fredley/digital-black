      ...
      $.ajax({
        ...
        success: () => {
          ...
          $(`[data-id=${id}]`).slideUp("fast", () => {
            if (
              parent.hasClass("aisle") &&
              parent.find("li:visible").length === 1
            ) {
              parent.slideUp("fast");
            }
          });
...
const reset_frequent = () => {
  ...
  frequent_items
    ...
    .forEach((item) => {
      ...
      $("#frequent").append(
        `<div data-name="${item.name}" class="item ${hidden}">${item.name}</div>`
      );
    });
  wire_frequent();
};
...
const display_frequent_with_filter = (term) => {
  ...
  $("#frequent").html("");
  frequent_items
    .filter((item) => item.name.toLowerCase().indexOf(term) >= 0)
    .filter((_, idx) => idx < 20)
    .forEach((item) => {
      const hidden = items.find((i) => i.name === item.name) ? "hidden" : "";
      $("#frequent").append(
...
const add_item = (name, cb) => {
  $.ajax({
    ...
    success: (data) => {
      $("#list").append(
        `<div data-id="${data}" class="item">${name}<button class='remove'><i class="fa fa-2x fa-check"></i></button></div>`
      );
      wire_remove();
      reflow_list();
      $("#input").val("");
...
const refresh_items = () => {
  $.ajax({
    ...
    success: (data) => {
      ...
      items.forEach((item) => {
        $("#list").append(
          `<div data-id="${item.id}" class="item">${item.name}<button class='remove'><i class="fa fa-2x fa-check"></i></button></div>`
        );
      });
      $("#frequent").html("");
      reset_frequent();
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      Object.keys(aisles).forEach((aisle) => {
        ...
        $("#shopping").append(
          $(
            `<ul class="aisle collapsed"><li class="title"><i class="fas fa-caret-right"></i> ${aisle}</li></ul>`
          ).append(rows.join(""))
        );
      });
        ...
        .on("mousedown touchstart", function () {
          ...
          press_timeout = setTimeout(() => {
            ...
            $("#editshade")
              .show()
              .find(".message")
              .text(`Update the aisle for ${item}:`);
